...
public class ImpresionTicketXComensal implements Printable {
...
//	protected final Component component = Component.getComponentAt(0,0) ;
	...
	int comensalNum =-1;
	double totalAImprimir = 0.00;
	double Descuento = 0.00;
	protected final int numeroDeCopasXBotanaGratis = 3;

    public ImpresionTicketXComensal (int IDMesa) throws Exception {
		this.IDMesa = IDMesa;
		totalComensales = mpv.dameComensalesPorMesa(IDMesa);
		validaReglas();
	...
	// HAGO DESCUENTO
						...
						if(comensalActual == rsDescuentos.getInt("numComensal")){
	...
	public void mandaImprimirTicketCT() throws Exception{
		...
		if(ContadorProductos > 0){
			hayElementos = true;
			while(rs.next()){
				totalAImprimir += rs.getDouble("precioTotal");
			}
			double total = 0.00;
			total = totalAImprimir - Descuento;
			total = mpv.cambiaADouble(mpv.formatoCortoDinero(total+""));
			PagoTotal = mpv.formatoDinero(mpv.acortaLongitud(total+""));
		}
		rs.beforeFirst();
	}
...
//ACTUALIZO PARA ESTE CLIENTE
	...
	public int print (Graphics g, PageFormat pageFormat, int page) throws PrinterException  {
		...
		pen.x = 105;
		g2d.drawString ("Cuenta: x Comensal", (int)pen.getX(),(int) pen.getY());
		pen.y +=(12);
		pen.x = 0;
		g2d.setFont (nuevaProd);
		...
		g2d.drawString ("Comen.:#"+comensalNum+"-"+NumComensales, (int)pen.getX(),(int) pen.getY());
		pen.x =(120);
