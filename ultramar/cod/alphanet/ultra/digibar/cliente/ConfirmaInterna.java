...
public class ConfirmaInterna extends JInternalFrame {

	private	JDesktopPane desktop = null;
	protected Vector <String> vPersonal;
	...
	protected JComboBox jcPersonal;
    ...
    public ConfirmaInterna() throws Exception {
        super("Confirmación Pedido Interno", false, true, false);
	...
	public void despliegaInterfaz() throws Exception{
		...
		url= getClass().getResource("/imas/fondoConfirmaComandaInterna.jpg");
		...
		Vector <Object> datos2 = cliente.bs.nuevaListaPInterno(datos);
		...
		cmodel.getColumn(9).setCellRenderer(cellAreaRenderer);
		...
		column.setMinWidth(134);
		column.setMaxWidth(134);
		column.setResizable(false);
		...
		vPersonal = cliente.bs.DameListadoEmpleados();
		jcPersonal = new JComboBox();
		for(int i = 0; i<vPersonal.size() ; i++){
			String nombre = cliente.ma.decodifica(vPersonal.elementAt(i));
			jcPersonal.addItem(nombre);
		}
		jcPersonal.setFont(letra);
		jcPersonal.setBounds(122,362,300,40);
...
//			System.out.println("Renglon Global Solicitado = "+renglonGlobalSolicitado +", renglonTrabajando = "+renglonTrabajando);
			...
			if(idTipoP == 1){
				if(tipoDeProducto.substring(tipoDeProducto.indexOf("*")+1, tipoDeProducto.lastIndexOf("*")).trim().equalsIgnoreCase(laClaveDeBotana+"")){
					...
					mcb.setTComponente((String)tmv.getValueAt(renglonGlobalSolicitado,9));
					...
					if(mcb.dioAceptar){
						...
						tmv.setValueAt(componentes,renglonTrabajando,9);
					}
				}
				else{
				...
				String tipoBebidaR = (String) tmv.getValueAt(renglonGlobalSolicitado, 5);
				String tipoCombina1 = (String)tmv.getValueAt(renglonGlobalSolicitado, 9);
				...
				if(!esCombinable && !esRedimensionable){
					...
					mcfb.setNombreBebida((String)tmv.getValueAt(renglonGlobalSolicitado,2));
					mcfb.setTipoBebida((String)tmv.getValueAt(renglonGlobalSolicitado,3));
					mcfb.setTCombina((String)tmv.getValueAt(renglonGlobalSolicitado,9));
					if(esCombinable){
						mcfb.setTFormato((String)tmv.getValueAt(renglonGlobalSolicitado,6));
						mcfb.setTMezcla((String)tmv.getValueAt(renglonGlobalSolicitado,7));
						mcfb.setTAdicional((String)tmv.getValueAt(renglonGlobalSolicitado,8));
					}
					...
					if(mcfb.dioAceptar){
						...
						if(esRedimensionable){
							...
							tmv.setValueAt(tamanioBebida,renglonTrabajando, 5);
						}
						if(esCombinable){
							...
							tmv.setValueAt(formatoBebida,renglonTrabajando, 6);
							if(tMezcla == 0){
								mezclaBebida = "NA";
							}
							else if(tMezcla == 1){
							...
							tmv.setValueAt(mezclaBebida,renglonTrabajando, 7);
							if(tAdicional == 0){
								adicionalBebida = "No";
							}
							if(tAdicional == 1){
							...
							tmv.setValueAt(adicionalBebida,renglonTrabajando, 8);
							...
							tmv.setValueAt(compuesto,renglonTrabajando,9);
						}
					}
				}
			}
	...
	public void procesoAceptar() {
		try{
			if(jtSolicitado.getRowCount() == 0){
				...
				String usuarioID = (String) jcPersonal.getSelectedItem();
				...
				if (!autentificacion){
					...
					Vector <String> resultadoAlmacena = cliente.bs.AlmacenaComandaInterna(tmv.datos, userID);
...
//							System.out.println("Entre al else de faltantes");
			...
			e.printStackTrace();
			JOptionPane.showInternalMessageDialog(cliente.desktop, "Excepcion  en ConfirmaInterna.\nVuelva a intentarlo.");
			Cierra();
		}
	}
