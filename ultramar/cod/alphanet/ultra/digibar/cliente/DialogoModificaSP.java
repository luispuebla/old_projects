...
public class DialogoModificaSP extends JDialog{
	...
	protected double precioBotella = 0.00;
	protected double precioCopa = 0.00;
	protected int cantidadElementos = -1;
	protected int  genero = -1, division = -1, claveProducto = -1;
	private Font letra;
	protected boolean primerCambio = true;
	protected int laCantidadActual = -1;
	protected JComboBox jcTipo;
	...
	public DialogoModificaSP (Frame padre, String titulo, boolean modal) throws Exception {
	...
	public void setProducto(int genero, int division, int claveProducto){
		this.genero = genero;
		this.division = division;
		this.claveProducto = claveProducto;
	}

	public void setCantidadActual(int laCantidadActual){
		this.laCantidadActual =  laCantidadActual;
	}
	...
	public void dibujaDialogo() throws Exception {
		...
		url= getClass().getResource("/imas/fonditoModificaProducto.jpg");
		...
		if (genero == 1){
			jcTipo.addItem("No Aplica");
		}
		// Averiguo si es por copa, copa o botella, solo botella.
		else{
			int tipoEnvase = cliente.bs.dameEntregaBebida (claveProducto, division);
			// tipoBebida-->  0 -> CopeoCon1Botella,  1 --> CopeoCon2 Botellas,  2-> SoloCopeo,   3->Solo1 Botella,  4 --> Las 2 Botellas.
			if(tipoEnvase == 0){
				jcTipo.addItem("Copa");
				jcTipo.addItem("Botella");
			}
			else if(tipoEnvase == 1){
				jcTipo.addItem("Copa");
				jcTipo.addItem("Botella G.");
				jcTipo.addItem("Botella P.");
			}
			...
			else if(tipoEnvase == 4){
				jcTipo.addItem("Botella G.");
				jcTipo.addItem("Botella P.");
			}
		}
		jcTipo.setFont(letra);
		jcTipo.setBounds(277,347,200,45);
		...
		jtCantidad.setText(laCantidadActual+"");
...
// Verifico que la Cantidad sea mayor a cero.
			if(cliente.ma.cambiaAInt(jtCantidad.getText()) ==0){
				...
				String elTipo = ((String)jcTipo.getSelectedItem()).trim();
				cantidadElementos = jcTipo.getItemCount();
				// Si es alimento simplemente regreso
				if(elTipo.equalsIgnoreCase("No Aplica")  || (elTipo.equalsIgnoreCase("Copa") && cantidadElementos == 1)){
					existioCambio = true;
					Cierra();
				}
				else if(elTipo.equalsIgnoreCase("Copa") && (cantidadElementos == 2 || cantidadElementos == 3) ){
					precioCopa = cliente.bs.damePrecioCopa(claveProducto, division);
					existioCambio = true;
					Cierra();
				}
				else if(elTipo.equalsIgnoreCase("Botella") || elTipo.equalsIgnoreCase("Botella G.") || elTipo.equalsIgnoreCase("Botella P.") ){
					// 0 --> Solo Botella   1--> Botella Grande,    2--> Botella Pequeña
					int clasificaBotella = -1;
					if(elTipo.equalsIgnoreCase("Botella")){
						clasificaBotella = 0;
					}
					else if (elTipo.equalsIgnoreCase("Botella G.")){
						clasificaBotella = 1;
					}
					else{
						clasificaBotella = 2;
					}
					boolean verificaDisponibilidad = cliente.bs.dameDisponibilidadBotella(claveProducto, division, jtCantidad.getText().trim(), clasificaBotella);
					if(verificaDisponibilidad){
						precioBotella = cliente.bs.damePrecioBotella(claveProducto, division, clasificaBotella);
						existioCambio = true;
						Cierra();
					}
					else{
						MensajeNotificacion mn = new MensajeNotificacion(cliente, "Aviso", true, "No tengo "+jtCantidad.getText() +" botellas en Inventario");
						mn.dibujaDialogo();
						jtCantidad.requestFocus();
						jtCantidad.selectAll();
						primerCambio = true;
