...
public class AltaMesa extends JInternalFrame {
	...
	protected Vector <String> vMeseras, vMesasL;
    ...
    public AltaMesa() throws Exception {
        super("Alta de Mesa", false, true, false);
	...
	protected void setMesaAlta(String mesaAlta){
	...
	public void despliegaInterfaz() throws Exception{
		...
		url= getClass().getResource("/imas/fondoAltaMesa.jpg");
...
//			System.out.println("Diste click en Aceptar");
			...
			if (!(comensales > 0)){
				JOptionPane.showInternalMessageDialog(cliente.desktop, "Debes dar una cantidad mayor a 0");
				jtCantidad.setText("");
			}
			else{
				...
				String resultado = cliente.bs.ProcesaAltaMesa(IDMesero, comensales, mesaID, listModel2);
				if(resultado.equalsIgnoreCase("OK")){
					cliente.refrescaMapa();
					Cierra();
				}
				else if(resultado.equalsIgnoreCase("MO")){
					JOptionPane.showInternalMessageDialog(cliente.desktop, "La mesa: "+mesaID +	", ya fue dada de alta por alguien más.\n Si desea modificarla empleé Editar Configuración Mesa.");
					cliente.refrescaMapa();
					Cierra();
				}
			...
			e.printStackTrace();
			JOptionPane.showInternalMessageDialog(cliente.desktop, "Excepcion  en Alta Mesa.\nVuelva a intentarlo.");
			Cierra();
		}
	}
