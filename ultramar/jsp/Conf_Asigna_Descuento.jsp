...
<html>
<head>
<title>Guardadas Cortesias</title>
...
<%
	...
	String comensales = request.getParameter("numComensales");
	int numComensales = data.cambiaAInt(comensales.trim());
	String[][] datos = new String[numComensales][2];
	for(int i=0; i<numComensales; i++){
		datos[i][0] = (i+1)+"";
		datos[i][1] = request.getParameter("descuento"+(i+1));
	}
	boolean exito = data.almacenaDescuento(datos, comanda, numComensales);
	if(exito){
%>
<h1>Se Asignaron correctamente los descuentos</h1>
<%
	}
	else{
%>	
<p>Existi&oacute; un problema al momento de asignar los descuentos. </h1>
<%}%>
