...
<html>
<head>
<title>Materia Prima Agregada</title>
...
<%
	...
	boolean exis = true;
	exis = mp.existe(nombre);
	
	if(exis){ %>
		<b class="roj">La Materia Prima ya se encuentra Registrada en la Base de Datos</b>		
<p>&nbsp;</p>
<p><a href="Alta_Materia_Prima.jsp" target="mainFrame" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('APadres','','imagenes/Regresar2.jpg',1)">
...
//System.out.println("llenado "+llenado);
	if (llenado==1){
		if(tipo.equalsIgnoreCase("1")){
			resultado = mp.add_Materia_Prima3(nombre, tipo, tipoA, costoKg, costoU, invKg, kgxu, invU);
		}
		if(tipo.equalsIgnoreCase("2")){
			resultado = mp.add_Materia_Prima4(nombre, tipo, costoB, costoB2, lxb, lxb2, invB, invB2, invC);
		}
		if(resultado){
%>
		<jsp:forward page="Menu_Materias_Primas.jsp" />
<% 
		}
		else{  %>
