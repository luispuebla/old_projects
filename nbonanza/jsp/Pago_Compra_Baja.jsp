...
<html>
<head>
<title>Selección de Pago para la Baja</title>
...
function OnSubmitForm(){
	if(form1.clavePago != null){
		var valor = 'Ninguno';
		for(var i=0; i<form1.clavePago.length; ++i){
			if(form1.clavePago[i].checked){
				valor = form1.clavePago[i].value;
			}
		}	
		if(valor == 'Ninguno'){
			alert("Debes seleccionar un pago para proseguir.");
			return false;
		}
		else{
			document.form1.action ="Baja_Pago.jsp";		
		}
	}
	else{
		return false;	
...
%>
<h1>Escoge el Pago que deseas dar de Baja:</h1>
<p></p>
<form name="form1" method="post" onSubmit="return OnSubmitForm();">
 <table width="520" border="1" align="center">
  <caption>
  PAGOS
DE ESTA TRANSACCI&Oacute;N
  </caption>
  <tr> 
    <td><div align="center"> <input type="radio" name="clavePago"  readonly="true" disabled> </div> </td>
    <th width="92">Clave Pago</th>
	<th width="240">Fecha en la que se Realizo</th>
    <th width="125">Monto de la<br/>Transacci&oacute;n </th>
   </tr>
<%
	ResultSet rs = pa.damePagosCompra(claveOrden);
	while(rs.next()){
%>
  <tr> 
  <td> <div align="center">
    <input type="radio" name="clavePago" value="<%= rs.getString("consecutivo")%>" onClick="MandaSubmit(document.forms[0])">
  </div></td>
