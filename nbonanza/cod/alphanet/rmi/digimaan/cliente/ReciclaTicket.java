...
public class ReciclaTicket extends JInternalFrame {

	private String claveProducto = new String("");
	private int claveTicket = 0;
	public JTextField  jtNumTicket;
    ...
    public ReciclaTicket() throws Exception {
        super("Modificar Ticket", false, true, false);
	...
	public void despliegaInterfaz() throws Exception{
		...
		url= getClass().getResource("/imas/fondoModificaTicket.jpg");
		...
		jtNumTicket = new JTextField(30);
		jtNumTicket.setHorizontalAlignment(JTextField.CENTER);
		jtNumTicket.setFont(letra);
		// Acomodo mis campos dentro de la ventana.
		jtNumTicket.setBounds(310,140,220,40);
	...
	public void procesoModificaTicket() throws Exception{
		claveTicket = ma.cambiaAInt(jtNumTicket.getText());
		if(claveTicket == 0){
			JOptionPane.showInternalMessageDialog(digiCliente.desktop, "Debes escribir el  número del Ticket.");
			jtNumTicket.setText("");
		}
		else{
			boolean resultado =  false;
			try{
				resultado = digiCliente.bs.ticketCandidato(claveTicket);
			}
			...
			if (!resultado){
				JOptionPane.showInternalMessageDialog(digiCliente.desktop, "El ticket: "+claveTicket +" fue rechazado por una de las siguientes causas:\n1. No existe.\n2. Ya fue Pagado.\n3. Es un crédito ya liquidado.");
				jtNumTicket.setText("");
			}
			else{
				boolean reingresoInv = false;
				try{
					reingresoInv = digiCliente.bs.reingresaInventario(claveTicket);
				}
				catch(RemoteException re){
					this.setVisible(false);
					this.dispose();
				...
				if(reingresoInv){
					ListaRecicla lr = new ListaRecicla();
					lr.setPadre(digiCliente);
					lr.setClaveTicket(claveTicket);
					lr.despliegaInterfaz();
					lr.setVisible(true);
					digiCliente.desktop.add(lr);
					lr.moveToFront();
					this.setVisible(false);
					digiCliente.desktop.remove(this);
					this.dispose();
					digiCliente.desktop.setSelectedFrame(lr);
					lr.setSelected(true);
				}
				else{
				JOptionPane.showInternalMessageDialog(digiCliente.desktop,"No se pudo modificar el Ticket, pues fallo el Reingreso del producto a Inventario.\nVuelva a Intentarlo.");
				jtNumTicket.setText("");
				}
			}
		}
		...
		public void mousePressed(MouseEvent e){
			try{
				...
				if(temporal == jlBoton1){
					String previo = jtNumTicket.getText();
					if (previo.length() < 7){
						jtNumTicket.setText(previo+"1");
					}
				}
				else if(temporal == jlBoton2){
					String previo = jtNumTicket.getText();
					if (previo.length() < 7){
						jtNumTicket.setText(previo+"2");
					}
				}
				else if(temporal == jlBoton3){
					String previo = jtNumTicket.getText();
					if (previo.length() < 7){
						jtNumTicket.setText(previo+"3");
					}
				}
				else if(temporal == jlBoton4){
					String previo = jtNumTicket.getText();
					if (previo.length() < 7){
						jtNumTicket.setText(previo+"4");
					}
				}
				else if(temporal == jlBoton5){
					String previo = jtNumTicket.getText();
					if (previo.length() < 7){
						jtNumTicket.setText(previo+"5");
					}
				}
				else if(temporal == jlBoton6){
					String previo = jtNumTicket.getText();
					if (previo.length() < 7){
						jtNumTicket.setText(previo+"6");
					}
				}
				else if(temporal == jlBoton7){
					String previo = jtNumTicket.getText();
					if (previo.length() < 7){
						jtNumTicket.setText(previo+"7");
					}
				}
				else if(temporal == jlBoton8){
					String previo = jtNumTicket.getText();
					if (previo.length() < 7){
						jtNumTicket.setText(previo+"8");
					}
				}
				else if(temporal == jlBoton9){
					String previo = jtNumTicket.getText();
					if (previo.length() < 7){
						jtNumTicket.setText(previo+"9");
					}
				}
				else if(temporal == jlBoton0){
					String previo = jtNumTicket.getText();
					if (previo.length() < 7){
						jtNumTicket.setText(previo+"0");
					}
				}
