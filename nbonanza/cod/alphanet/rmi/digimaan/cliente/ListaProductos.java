...
public class ListaProductos extends JInternalFrame {
	...
	private ImageIcon iiBotonCanN, iiBotonCanP, iiBotonAD, iiBotonAcepN,iiBotonAcepP, iiBotonBorrarN, iiBotonBorrarP;
	private Sound son;
	...
	protected static double topePago  = 500.00;
    ...
    public ListaProductos() throws Exception {
	...
	public void despliegaInterfaz() throws Exception{
		...
		try{
			existencia = digiCliente.bs.existenProductosEnLista(claveTicket);
		}
		catch (RemoteException re){
			this.setVisible(false);
			this.dispose();
		...
		try{
			datosTabla = new JTable(digiCliente.bs.tablaListadoPrevio(claveTicket));
		}
		catch (RemoteException re){
			this.setVisible(false);
			this.dispose();
		...
		url= getClass().getResource("/imas/fondoListadoVenta.jpg");
		...
		jlCancela.setBounds(120,510,165,70);
		...
		try{
			jtProductos.setText(digiCliente.bs.cantidadProductos(claveTicket));
		}
		catch (RemoteException re){
			this.setVisible(false);
			this.dispose();
		...
		try{
			totalAPagar = digiCliente.bs.sumaPagoProductos(claveTicket);
		}
		catch (RemoteException re){
			this.setVisible(false);
			this.dispose();
...
// Recibo quien procesa el ticket..
    ...
    */
	...
	protected void ProcesoAceptar() throws Exception{
		...
		try{
			procesaCierre = digiCliente.bs.cierraCuenta(claveTicket, rfc, nombreCliente, claveU);
		}
		catch (RemoteException re){
			this.setVisible(false);
			this.dispose();
		...
		try{
			usuarioAutorizado = digiCliente.bs.dameAutorizacionUser(claveU);
		}
		catch (RemoteException re){
			this.setVisible(false);
			this.dispose();
		...
		if(procesaCierre){
			if(!usuarioAutorizado && rfc.length() == 0){
			...
			else if(usuarioAutorizado && rfc.length() == 0) {
				ConfirmaUsuarioA cu = new ConfirmaUsuarioA();
				cu.setPadre(digiCliente);
				cu.setClaveTicket(claveTicket);
				cu.setUsuario(claveU);
				cu.despliegaInterfaz();
				cu.setVisible(true);
				digiCliente.desktop.add(cu);
				cu.moveToFront();
				this.setVisible(false);
				digiCliente.desktop.remove(this);
				this.dispose();
				digiCliente.desktop.setSelectedFrame(cu);
				cu.setSelected(true);
			}
			else{
				ImpresionTicketRem it = new ImpresionTicketRem();
			...
			JOptionPane.showInternalMessageDialog(digiCliente.desktop, "No se puede cerrar la cuenta, cheque la conexion al Servidor");
		}
	}
