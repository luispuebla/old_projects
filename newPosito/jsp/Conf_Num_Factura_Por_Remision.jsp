...
<%
	...
	if(nombreFacEnBlanco){ %>
		...
		<form name="form1" method="post" action="Num_Factura_Por_Remision.jsp">
...
<%		}
			...
			int almacena = factura.almacenaFacturas(rfc, nombre, ArrayNumTicket, ArrayFactura, fechaDeFactura, produEnFac);
			if(almacena == -1){%>
...
<%			}
				...
				ResultSet rs = factura.dameListadoTicketsFac(ArrayNumTicket);			
				for(int i=0; i<ArrayFactura.length; i++){
					...
					%>
					<form name="formFac" action="Print_Fac_Por_Remision.jsp" method="post" target="_blank">
						<input type="hidden" name="fechaFac" value="<%=fechaDeFactura%>">
						<%	for(int j=0; j < ArrayNumTicket.length; j++){ %>
	    					<input type="hidden" name="numTicket" value="<%=ArrayNumTicket[j]%>">
						<%	} %>	
