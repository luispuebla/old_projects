<%@ page contentType="text/html; charset=iso-8859-1" language="java" import="java.sql.*, java.text.*, java.util.Vector, java.math.*" errorPage="" %>
...
<title>Modificaci&oacute;n de Compra</title>
...
function OnSubmitForm(){
	if(document.pressed == 'Guardar')   {
		...
		document.ph1.action ="ListadoCompraM.jsp";
	}
	else if(document.pressed == 'Regalo1'){
		document.ph1.action ="ListadoCompraOM.jsp?"+"1";
	}
	else if(document.pressed == 'Regalo2'){
		document.ph1.action ="ListadoCompraOM.jsp?"+"2";
	}
	else if(document.pressed == 'Regalo3'){
		document.ph1.action ="ListadoCompraOM.jsp?"+"3";
	}
	return true;
}
...
<%
	...
	if(request.getParameter("claveOrden") != null){
		...
		if(request.getParameter("descuento1") != null){
			descuento1 = request.getParameter("descuento1");
			descuento2 = request.getParameter("descuento2");
			descuento3 = request.getParameter("descuento3");
			fechaRegistro = request.getParameter("fechaRegistro");
		...
		if(request.getParameter("inicio") != null){ // LLeno por primera vez las variables correspondientes de productos
			ResultSet rsDatosCompra = alCom.dameDatosCompra(claveOrden);
			while(rsDatosCompra.next()){
				descuento1 = rsDatosCompra.getString("descuento1");
				descuento2 = rsDatosCompra.getString("descuento2");
				descuento3 = rsDatosCompra.getString("descuento3");
				fechaRegistro = rsDatosCompra.getString("fechaRegistro");
				fechaEntrega = rsDatosCompra.getString("fechaEntrega");
				subtotal = rsDatosCompra.getString("subtotal");
				iva = rsDatosCompra.getString("iva");
				descuentos = rsDatosCompra.getString("descuento");
				total = rsDatosCompra.getString("total");
				observaciones = rsDatosCompra.getString("observaciones");
			}
			rsDatosCompra.close();
		...
		if(!(ArrayCantidad[0].trim().equals("")) && contadorInterno > 0){ // No esta vacio el arreglo previo y trae algo de ListadoCompra.jsp
			...
			if(vTemp.size() > 0){
				...
				for (int i = 0; i<vTemp.size(); i++){
					...
					TempArrayCUnidad[i+ArrayCantidad.length] = (alCom.precioAsignado(TempArrayProducto[i+ArrayCantidad.length], TempArrayTipo[i+ArrayCantidad.length])+"");
...
<h1>FORMATO SOLICITUD MODIFICACI&Oacute;N DE COMPRA.</h1>
...
<caption> Proveedor: </caption>
  <tr>
    <th width="180"><div align="center"> Nombre Proveedor:</div></th>
	<td width="284"> <div align="center"><%= prov.nombreProveedor(proveedor)%><input type="hidden" name="proveedorC" value="<%=proveedor%>"></div></td>
  </tr>
</table>
<br />
<table width="700" border="1" cellspacing="0" cellpadding="0">
  <caption align="top">
   MODIFICACI&Oacute;N DE COMPRA 
  </caption>
  <tr>
...
<%
	...
	for (int i=0; i < ArrayCantidad.length; i++){
		...
		BigDecimal totalpro = new BigDecimal(tempCantidad);
		totalpro = totalpro.setScale(3, RoundingMode.HALF_UP);					
		tempCantidad = alCom.dameEnteros(totalpro.toString());		
		...
		if(ArrayTipo[i].equals("PZ") || ArrayTipo[i].equals("Pieza"))
			...
			tempTipo = "Kilo";
			tempCantidad = totalpro.toString();							
		}
...
<table align="center" width="600" border="1">
	...
	<tr>
		...
		<td><input type="text" size="8" maxlength="6" onFocus="seleccionaTodo(this)" name="cantidadRegalo1" <% if(prodRegalo1.equals("")){%> readonly="true" <% }%> 
<% 
	if(! prodRegalo1.equals("")){
		BigDecimal cantidadReg = new BigDecimal(cantidadRegalo1);
		cantidadReg = cantidadReg.setScale(3, RoundingMode.HALF_UP);					
		cantidadRegalo1 = alCom.dameEnteros(cantidadReg.toString());		
		if (tipoUniRegalo1.equals("Kilo")) {
			cantidadRegalo1 = cantidadReg.toString();
		} 
	}
%>	
		value="<%=cantidadRegalo1%>" onKeyPress="	
	<% if(tipoUniRegalo1.equals("Kilo")){ %>
	   if (event.keyCode < 45 || event.keyCode > 57) {event.returnValue = false;}
	...
	<tr>
		...
		<td><input type="text" size="8" maxlength="6" onFocus="seleccionaTodo(this)" name="cantidadRegalo2" <% if(prodRegalo2.equals("")){%> readonly="true" <% }%>
<% 
	if(! prodRegalo2.equals("")){
		BigDecimal cantidadReg = new BigDecimal(cantidadRegalo2);
		cantidadReg = cantidadReg.setScale(3, RoundingMode.HALF_UP);					
		cantidadRegalo2 = alCom.dameEnteros(cantidadReg.toString());		
		if (tipoUniRegalo2.equals("Kilo")) {
			cantidadRegalo2 = cantidadReg.toString();
		} 
	}
%>			
	 value="<%=cantidadRegalo2%>" onKeyPress="	
	<% if(tipoUniRegalo2.equals("Kilo")){ %>
	   if (event.keyCode < 45 || event.keyCode > 57) {event.returnValue = false;}
	...
	<tr>
		...
		<td><input type="text" size="8" maxlength="6" onFocus="seleccionaTodo(this)" name="cantidadRegalo3" <% if(prodRegalo3.equals("")){%> readonly="true" <% }%>
<% 
	if(! prodRegalo3.equals("")){
		BigDecimal cantidadReg = new BigDecimal(cantidadRegalo3);
		cantidadReg = cantidadReg.setScale(3, RoundingMode.HALF_UP);					
		cantidadRegalo3 = alCom.dameEnteros(cantidadReg.toString());		
		if (tipoUniRegalo3.equals("Kilo")) {
			cantidadRegalo3 = cantidadReg.toString();
		} 
	}
%>					
	 value="<%=cantidadRegalo3%>" onKeyPress="
	<% if(tipoUniRegalo3.equals("Kilo")){ %>
	   if (event.keyCode < 45 || event.keyCode > 57) {event.returnValue = false;}
