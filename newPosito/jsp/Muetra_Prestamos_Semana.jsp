...
<%	String semana = request.getParameter("semana");
	...
	ResultSet rs = empleado.dameTodosPrestamos(semana);
	BigDecimal totalPrestado = new BigDecimal("0.00");
...
%>
	...
	<caption>Registro de pr&eacute;stamos del <%=de%> al <%=a%></caption>
<%			
		while(rs.next()){
			if(nombreVendedor.equals("")){
				...
				totalPrestado = totalPrestado.add(new BigDecimal(rs.getString("cantidadAPagar")));
%>

	  <tr>
	  	<th width="136">Empleado:</th>
		<td colspan="4"><div align="center"><%=nombreVendedor%></div></td>
	  </tr>
	  <tr>
		<th>Clave Pr&eacute;stamo:</th>
		<th width="168">Tipo de Pr&eacute;stamo</th>
		<th width="172">Cantidad A Pagar</th>
		<th width="162">Fecha de Pr&eacute;stamo</th>		
		<th width="100">Autoriz&oacute;</th>				
	  </tr>
	  <tr>
		<td><div align="center">
	      <% if (rs.getString("tipo").equals("producto")){%>
	      <a href="DetalleTicket.jsp?<%=rs.getString("ID")%>*<%=de%>^<%=a%>"><%=rs.getString("ID")%></a>
	      <%} else{ %>
	      <%=rs.getString("ID")%>
	      <%}%>
		  </div></td>
		...
		<td><div align="center"><%=rs.getString("autorizo")%></div></td>
	  </tr>
<%			} // Cierro if
			else if(nombreVendedor.equals(rs.getString("nombreVendedor"))){ 
				prestadoXEmpleado = prestadoXEmpleado.add(new BigDecimal(rs.getString("cantidadAPagar")));
				totalPrestado = totalPrestado.add(new BigDecimal(rs.getString("cantidadAPagar"))); 
%>
	  <tr>
...
%>
	  <tr>
		...
		<td colspan="3"><div align="center"><b>$ <%=empleado.formatoDinero(prestadoXEmpleado.toString())%></b></div></td>
	  </tr>
	  <tr>
	  	<td colspan="5">&nbsp;</td>
	  </tr>
...
<%			} // Cierro else if
		...
		rs.close();
		prestadoXEmpleado = prestadoXEmpleado.setScale(2, RoundingMode.HALF_UP);
		totalPrestado = totalPrestado.setScale(2, RoundingMode.HALF_UP);
%>
	  <tr>
	  	<th colspan="2">TOTAL PARCIAL PRESTADO:</th>
		<td colspan="3"><div align="center"><b>$ <%=empleado.formatoDinero(prestadoXEmpleado.toString())%></b></div></td>
	  ...
	  <tr>
	  	<th colspan="3" height="30">GRAN TOTAL A PR&Eacute;STAMO</th>
		<td colspan="2"><div align="right"><b>$ <%=empleado.formatoDinero(totalPrestado.toString())%></b></div></td>
	  </tr>		
	</table>
<%	}
	else{ %>
		<b class="roj">No hay registro de pr&eacute;stamos en la semana del <%=de%> al <%=a%></b>
<%	} 
