<%@ page contentType="text/html; charset=iso-8859-1" language="java" import="java.sql.*" errorPage="" %>
<jsp:useBean id="pa" class="com.alpha.bd.ManagerPagos" scope="page" />
<jsp:useBean id="prov" class="com.alpha.bd.ManagerProveedores" scope="page" />
<jsp:useBean id="com" class="com.alpha.bd.ManagerCompra" scope="page" />

<html>
<head>
<title>Pagos por Compra</title>
<link href="baseCSS.css" rel="stylesheet" type="text/css">
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
<script language="JavaScript" type="text/JavaScript">
<!--
function MM_swapImgRestore() { //v3.0
  var i,x,a=document.MM_sr; for(i=0;a&&i<a.length&&(x=a[i])&&x.oSrc;i++) x.src=x.oSrc;
}

function MM_preloadImages() { //v3.0
  var d=document; if(d.images){ if(!d.MM_p) d.MM_p=new Array();
    var i,j=d.MM_p.length,a=MM_preloadImages.arguments; for(i=0; i<a.length; i++)
    if (a[i].indexOf("#")!=0){ d.MM_p[j]=new Image; d.MM_p[j++].src=a[i];}}
}

function MM_findObj(n, d) { //v4.01
  var p,i,x;  if(!d) d=document; if((p=n.indexOf("?"))>0&&parent.frames.length) {
    d=parent.frames[n.substring(p+1)].document; n=n.substring(0,p);}
  if(!(x=d[n])&&d.all) x=d.all[n]; for (i=0;!x&&i<d.forms.length;i++) x=d.forms[i][n];
  for(i=0;!x&&d.layers&&i<d.layers.length;i++) x=MM_findObj(n,d.layers[i].document);
  if(!x && d.getElementById) x=d.getElementById(n); return x;
}

function MM_swapImage() { //v3.0
  var i,j=0,x,a=MM_swapImage.arguments; document.MM_sr=new Array; for(i=0;i<(a.length-2);i+=3)
   if ((x=MM_findObj(a[i]))!=null){document.MM_sr[j++]=x; if(!x.oSrc) x.oSrc=x.src; x.src=a[i+2];}
}

function MandaSubmit(f){
	document.retache.submit();
}


//-->
</script>
</head>
<body onLoad="MM_preloadImages('imagenes/Regresar2.jpg')">
<%
 	String usuario = (String) session.getAttribute("UsuarioActual");
 	String tipo = (String) session.getAttribute("TipoUsuario");
	if (usuario == null || !tipo.equalsIgnoreCase("administrador"))  {
%>	
		<jsp:forward page="RedirectAcceso2.html" />
<%
	}
	String queryString = request.getQueryString(); 
	String claveOrden = queryString.substring(0,queryString.indexOf("*"));
	String tipoConsulta = queryString.substring(queryString.indexOf("*")+1,queryString.lastIndexOf("*"));
	String fecha = queryString.substring(queryString.lastIndexOf("*")+1,queryString.indexOf("!"));
	String totalCompra = queryString.substring(queryString.indexOf("!")+1 ,queryString.length());
%>

  <table border="1" align="center" width="750">
    <tr>
		<th width="393">Faltante por Pagar:</th>
		<th width="341" height="40"><b class="aviso">$ <%=pa.formatoDinero(totalCompra)%></b></th>		
	</tr>
  </table>
  <p></p>
 <table width="750" border="1" align="center">
  <caption> Compra </caption>
<%
	ResultSet rs1 = com.datosOrdenCompra2(claveOrden);
	while(rs1.next()){
%>  
  <tr> 
    <th colspan="7">Clave de Compra</th>
  </tr>	
  	<td colspan="7"><div align="center"><%=rs1.getString("claveOrden")%></div></td>
  <tr>  
	<th width="77">Desc. 1</th>
    <th width="78">Desc. 2</th>
    <th width="79">Desc. 3</th>
    <th width="116">SubTotal</th>
    <th width="116">Descuento</th>
    <th width="116">IVA</th>
    <th width="122">Total</th>
   </tr>
  <tr> 
	<td><div align="center"><%=rs1.getString("descuento1")%> %</div></td>
    <td><div align="center"><%=rs1.getString("descuento2")%> %</div></td>
    <td><div align="center"><%=rs1.getString("descuento3")%> %</div></td>
    <td><div align="right">$ <%=pa.formatoDinero(rs1.getString("subtotal"))%></div></td>
    <td><div align="right">$ <%=pa.formatoDinero(rs1.getString("realDescuento"))%></div></td>
    <td><div align="right">$ <%=pa.formatoDinero(rs1.getString("iva"))%></div></td>
    <td><div align="right">$ <%=pa.formatoDinero(rs1.getString("total"))%></div></td>
  </tr>
  <tr>
   <th colspan="3">Fecha Registro</th>
   <th colspan="2">Fecha &Uacute;ltima Entrega</th>
   <th colspan="2">Fecha Pago</th>
  </tr>
  <tr>
    <td colspan="3"><div align="center"><%=rs1.getString("fechaRegistro")%></div></td>
	<td colspan="2"><div align="center"><%=rs1.getString("fechaEntrega")%> </div></td>
    <td colspan="2"><div align="center"><%=rs1.getString("fechaPago")%> </div></td>  
  </tr>
<%
	}
	rs1.close();
	com.cierraConexion();
%>	  
</table>
<p></p>
 <table width="750" border="1" align="center">
  <caption>   PAGOS   </caption>
  <tr> 
    <th>Clave</th>
    <th>Compra</th>
	<th>Fecha</th>
    <th>Monto</th>
    <th>Forma de Pago</th>
    <th>Cheque / Referencia</th>
   </tr>
<%
	ResultSet rs = pa.damePagosCompra2(claveOrden);
	while(rs.next()){
%>
  <tr> 
    <td><div align="center"><%=rs.getString("consecutivo")%></div></td>
    <td><div align="center"><%=rs.getString("claveOrden")%></div></td>
	<td><div align="center"><%=rs.getString("fechaPago")%></div></td>
    <td><div align="right">$ <%=pa.formatoDinero(rs.getString("monto"))%></div></td>
    <td><div align="center">
        <%
	if(rs.getInt("formaPago") == 1)
		out.print("Efectivo");
	if(rs.getInt("formaPago") == 2)
		out.print("Cheque");
	if(rs.getInt("formaPago") == 3)
		out.print("Depósito");
	if(rs.getInt("formaPago") == 4)
		out.print("Tarjeta de Crédito");
	if(rs.getInt("formaPago") == 5)
		out.print("Tarjeta de Débito");
	
	%>
    </div></td>
    <td><div align="center"><%=rs.getString("numeroCheque")%> &nbsp;</div></td>
  </tr>
<%
	}
	rs.close();
	pa.cierraConexion();
%>	  
</table>
<form name="retache" method="post" target="_self" action="PagosParaHoy.jsp">
	<input type="hidden" name="tipoConsul" value="<%=tipoConsulta%>">
	<input type="hidden" name="datebox1" value="<%=fecha%>">

<p align="center"><a href="#" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Retache','','imagenes/Regresar2.jpg',1)"><img src="imagenes/Regresar1.jpg" name="Retache" width="132" height="41" border="0" onClick="MandaSubmit(this.form)"></a></p>
</form>
<p align="center"><img src="imagenes/barraN.jpg" width="532" height="82"></p>
</body>
</html>
