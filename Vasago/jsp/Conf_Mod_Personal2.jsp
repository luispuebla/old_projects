...
<%
	...
	String elTipo = request.getParameter("tipo2");
...
<%
	...
	if(entrada && iguales){
		boolean guardado = personalM.update_Usuarios();
		if (guardado){
%>
...
%>
<form method="post" name="formaR" action="Conf_Mod_Personal.jsp">
<input type="hidden" name="claveu" value="<%=elUsuario%>">
...
<input type="hidden" name="elPuesto" value="<%=elPuesto%>">
<input type="hidden" name="uno" value="falto">
<%	
		if(!entrada){
%>
 <p align="center"><font color="#FF0000">Se deben llenar todos los campos para dar un cambio de Usuario, Da un click en Regresar para reingresar todos los datos.</font></p>		
<%		
		}
		else if(!iguales){
%>
