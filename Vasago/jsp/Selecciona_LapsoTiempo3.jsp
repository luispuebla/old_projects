...
<html>
<head>
<title>Selecci&oacute;n de  Periodo</title>
...
function OnSubmitForm(){
	...
	if(fecha > fecha2){
		...
		encontro = true;
		document.calform.action ="VentasXCliente_ReImp.jsp";				
	}
	return encontro;
}
...
<%
	...
	claveCliente = request.getParameter("clave");	
	if(claveCliente == null){
%>
	<b class="aviso">Se debe seleccionar un cliente de la lista previa.</b>
<%
	}
	else{
		if(claveCliente.equals("$todos$")){
			claveCliente = "%";
			nombreCliente = "Todos los Clientes Activos";
		}
%>	
<jsp:setProperty name="alVen" property="claveCliente" value="<%=claveCliente%>"/>
<%		
		if(! claveCliente.equals("%")){
			nombreCliente = alVen.dameNombreCliente();
		}		
%>
<body onLoad="setFecha()">
  <h1>Selecciona el lapso de tiempo para mostrar las<br /> 
  ventas del cliente:<br />
  <br /><%=nombreCliente %></h1>
...
<p align="center">
	<input type="hidden" name="clave" value="<%=claveCliente%>">
	<input type="hidden" name="nombreP" value="<%=nombreCliente%>">
    <input type="submit" name="Submit" value="Enviar">
</p>
</form>
<%}%>
<p align="center"><font color="#FFFFFF"><a href="Escoge_Cliente_Faltantes_Re.jsp" target="_self" 
