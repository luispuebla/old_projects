...
*	@param 	direccion	String con la Dirección del Cliente.
*/
	public boolean add_Cliente(String rfc, String nombre, String direccion, String colonia, String delegacion, String cp, String ciudad, String tipo0, String tel1, String tel2, String fax, String email, String diaCierran, String horaComida, String contacto, String credito, String pagoA, String agente) throws SQLException, Exception {
		//System.out.println("agente funcion "+agente);
		boolean fin = false;
		credito = remueveComas(credito);
		int cierran = cambiaAInt(diaCierran);
		...
		if (con.miConnection != null) {
			try {
				...
				updateCliente.setString(3, direccion);
				updateCliente.setString(4, colonia);
				updateCliente.setString(5, delegacion);
				updateCliente.setString(6, cp);
				updateCliente.setString(7, ciudad);
				updateCliente.setInt(8, tipo);
				...
				updateCliente.setInt(13, cierran);
				updateCliente.setString(14, horaComida);
				updateCliente.setString(15, contacto);
				updateCliente.setDouble(16, cre);
				updateCliente.setInt(17, paga);
				updateCliente.setString(18, agente);
				updateCliente.setString(19, hoy);
				updateCliente.setInt(20, 1);
				updateCliente.setDouble(21, cre);
				updateCliente.setInt(22, 0);
...
*/
  public boolean update_Cliente(String rfc, String nombre, String direccion, String colonia, String delegacion, String cp, String ciudad, String tipo0, String tel1, String tel2, String fax, String email, String diaCierran, String horaComida, String contacto, String credito, String pagoA, String agente) throws SQLException, Exception {
	  boolean fin = true;
	  ResultSet rs = null;
	  con = new ConexionBD();
	  credito = remueveComas(credito);
	  ...
	  int tipo = cambiaAInt(tipo0);
            try  {
				String queryCrePrev = "SELECT credito FROM clientes WHERE rfc = '"+rfc+"' AND activo = 1";
				Statement st = con.miConnection.createStatement();
	  	  		rs = st.executeQuery(queryCrePrev);
	  	  		while (rs.next()){
					credPrev = rs.getDouble("credito");
				}
				String queryString = "UPDATE clientes SET nombre = '"+nombre+"', direccion = '"+direccion+"', colonia = '"+colonia+"', delegacion = '"+delegacion+"', cp = '"+cp+"', ciudad = '" +ciudad+"', tipo = " +tipo+", tel1 = '" +tel1+"', tel2 = '" +tel2+"', fax = '" +fax+"', email = '" +email+"',  diaCierran = " +cierran+", horaComida = '" +horaComida+"', contacto = '" +contacto+"', credito = " +cre +", saldoDisponible = saldoDisponible + "+(cre - credPrev) +", pagoA = " +paga+", agente = '" +agente+"' WHERE rfc = '"+rfc+"' AND activo = 1" ;
              Statement stmt = con.miConnection.createStatement();
              stmt.execute(queryString);
            } catch (SQLException sqle) {
				sqle.printStackTrace();
	...
	public void cierraConexion() throws Exception{
       if(con != null){
			try{
		   		...
		   		System.out.println("Error al cerrar conexion de ManagerClientes");
		   		q.printStackTrace();
		  	}
		}
	}
...
/**
* El método ClientesXAgente se encarga de enviar todos los datos registrados dentro de la Base de Datos correspondientes a todos los Clientes que se encuentren activos, es decir que no hayan sido dados de baja.
*	@return rs 			ResultSet que contiene todos los datos correspondiente a todos los Clientes.
*/
  public ResultSet ClientesXAgente(String agente) throws SQLException, Exception {
          ResultSet rs = null;
          String estatus = "1";
          con = new ConexionBD();
          try  {
            String queryString = ("SELECT * FROM clientes WHERE agente = '"+agente+"' AND activo = '"+estatus+"' ORDER BY nombre");
            Statement stmt = con.miConnection.createStatement();
            rs = stmt.executeQuery(queryString);
          } catch (SQLException sqle) {
            error = "SQLException: Could not execute the query ClientesXAgente.";
            throw new SQLException(error);
          } catch (Exception e) {
            error = "An exception occured while retrieving ClientesXAgente.";
            throw new Exception(error);
          }
          return rs;
  }
