...
public class ImpresionTicketD implements Printable {
...
// VARIABLES
	private String tienda = "   \" EL  RETOÑITO \" ";
    ...
    public ImpresionTicketD(int claveTicket, String identificador, int usuario, String elPago,String elCambio) throws Exception {
		this.elCambio = elCambio;
		this.elPago = elPago;
...
//			System.out.println("i--> "+i+" "+services[i].getName());
	...
	public int print (Graphics g, PageFormat pageFormat, int page) throws PrinterException  {
		...
		g2d.drawImage (image,(int)(POINTS_PER_INCH * 1.879), 0, (int) (0.875 * POINTS_PER_INCH),  (int) (0.833 * POINTS_PER_INCH), frame);
		//--- Creo el String inicial con la Fecha y Numero de Ticket.
		// Inicializo mi punto para realizar los trazos, mando imprimir "El retoñito"
		Point2D.Double pen = new Point2D.Double (POINTS_PER_INCH * 0.380, 0.0);
		g2d.setFont (nuevaTitulo);
		...
		pen.y += fontNormalDescent +  fontNormalLeading; // Sumo a mi punto y lo que sale de la letra y el espacio entre renglones
		pen.x = (0.365 * POINTS_PER_INCH);
		// Bajo mi coordinada de acuerdo a la altura de la letra.
		pen.y += fontNormalAscent;
		g2d.drawString ("Ticket Único de Venta", (int)pen.getX(),(int) pen.getY());
		// ESTO SE REPITE PARA CADA RENGLON!:
		...
		if((page+1) == (int)Math.ceil(cantidadHojas)){
			try{
				...
				pen.y += fontBoldDescent +  fontBoldLeading;
				pen.y += fontBoldAscent;
				pen.x = (POINTS_PER_INCH * 1.350);
				g2d.drawString ("Pago........ $ ", (int)pen.getX(),(int) pen.getY());
				pen.x = (POINTS_PER_INCH * 2.100);
				if(mpv.formatoDinero(elPago).length() <= 6){
					pen.x += -3;
				}
				g2d.drawString (mpv.formatoEspaciado2(mpv.formatoDinero(elPago)), (int)pen.getX(),(int) 	pen.getY());
				pen.y += fontBoldDescent +  fontBoldLeading;
				pen.y += fontBoldAscent;
				pen.x = (POINTS_PER_INCH * 1.350);
				g2d.drawString ("Cambio.... $ ", (int)pen.getX(),(int) pen.getY());
				pen.x = (POINTS_PER_INCH * 2.100);
				if(mpv.formatoDinero(elCambio).length() <= 6){
					pen.x += -3;
				}
				g2d.drawString (mpv.formatoEspaciado2(mpv.formatoDinero(elCambio)), (int)pen.getX(),(int) pen.getY());
			}
			catch (Exception et){
				et.printStackTrace();
			}
